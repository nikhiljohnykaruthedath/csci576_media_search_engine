...
def display_plot(final_scores, title, save_location=None):
    for i, score in enumerate(final_scores):
        ...
        if i == 0:
            plt.plot(range(Y.shape[0]), Y, linewidth=2)
            plt.stackplot(range(y.shape[1]), y, labels=labels, colors=colors, alpha=0.5)
        plt.plot(np.argmax(Y), score[1], marker='d', markersize=7, label=score[0], color='black')
        plt.annotate("{}\n({})".format(score[0], np.argmax(Y)), (np.argmax(Y), score[1]), textcoords="offset points", xytext=(0,-30), ha='center', size='x-small')
    plt.gca().axes.get_xaxis().set_visible(False)
    if save_location is not None:
        plt.savefig(os.path.join(save_location,
                                 title+'_final.png'), dpi=600)
...
def generate_plots(final_scores, title, save_location=None):
    ...
    # plt.title(title)
    if save_location is not None:
        plt.savefig(os.path.join(save_location, title+'_stacked.png'), dpi=600, bbox_inches='tight', pad_inches = 0)
