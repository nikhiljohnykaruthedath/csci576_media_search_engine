...
class VideoPlayer(tk.Frame):
    ...
    def __init__(self, master, video_obj=None):
        ...
        self.play_pause_btn = ttk.Button(
            self, text='PLAY', command=self.play_pause
        )
        ...
        self.seek_frame = tk.LabelFrame(self, text='')
        self.seek_frame.pack(side='bottom', expand=True, fill='both')
        self.seek_bar = tk.Scale(
            self.seek_frame, from_=0, to=1,
            orient=tk.HORIZONTAL, command=self.seek, length=350,
            showvalue=0
        )
